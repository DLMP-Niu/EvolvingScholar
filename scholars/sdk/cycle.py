"""scholars/sdk/cycle.py — advance one experiment run's A→B→C loop by one step.
...
`feedback_project.yaml` to advance. Reuses the engine (Loop A) + harness (pi,
loop_c) functions.

    python scholars/sdk/cycle.py                       # start a NEW run (Loop A)
    python scholars/sdk/cycle.py <run_dir>             # advance THAT run one step
    python scholars/sdk/cycle.py <run_dir> --skip      # no PI feedback: capture only, no Loop C
...
State is derived from files (no new state store):
    ...
    (--skip)            → mark the run captured-but-not-evolved and stop (ADR-0014)
"""
...
def _run_no(meta: dict) -> int:
    """This run's feedback-gated index; tolerate the retired `cycle` field on old runs."""
...
def _stop_for_pi(run_dir: Path, out: dict[str, Path]) -> None:
    ...
    print(f"  or:    python scholars/sdk/cycle.py {run_dir} --skip   (if no PI feedback for this run)")


def _mark_skipped(run_dir: Path, meta: dict) -> None:
    """Record a no-feedback run as captured-but-not-evolved (ADR-0014): a supported
    outcome, not a stuck state. No Loop C runs; the store is untouched."""
...
def advance(run_dir: Path | None, project: str = DEFAULT_PROJECT,
    ...
    if run_dir is None:
        ...
        _, run_dir = asyncio.run(run_project(seed_question(project, cohort),
                                             project=project, cohort=cohort))
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(description="Advance one experiment run's A→B→C loop by one step.")
